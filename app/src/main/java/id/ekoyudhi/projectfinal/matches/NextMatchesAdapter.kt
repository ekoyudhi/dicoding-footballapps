...
import id.ekoyudhi.projectfinal.R.color.colorPrimaryText
...
class NextMatchesAdapter(private val eventLeagueList : List<EventLeague>, private val listener : (EventLeague) -> Unit)
    : RecyclerView.Adapter<NextMatchesViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NextMatchesViewHolder {
        return NextMatchesViewHolder(NextMatchesUI().createView(AnkoContext.create(parent.context,parent)))
    }

    override fun getItemCount(): Int = eventLeagueList.size

    override fun onBindViewHolder(holderNext: NextMatchesViewHolder, position: Int) {
        holderNext.bindItem(eventLeagueList[position], listener)
    }
}
                    ...
                    linearLayout {
                        ...
                        relativeLayout {
                            ...
                            imageButton {
                                ...
                                setBackgroundResource(R.color.cardview_shadow_end_color)
                            }.lparams {
                                marginEnd = dip(15)
                                width = wrapContent
                                height = wrapContent
...
class NextMatchesViewHolder(view : View) : RecyclerView.ViewHolder(view) {
    ...
    private val btnAlarm : ImageButton = view.find(alarm_button)
    fun bindItem(eventLeague: EventLeague, listener: (EventLeague) -> Unit) {
        ...
        btnAlarm.setOnClickListener {
            //snackbar(it, date.toString()).show()
            val intnt = Intent(Intent.ACTION_INSERT)
                    .setData(CalendarContract.Events.CONTENT_URI)
                    .putExtra(CalendarContract.EXTRA_EVENT_BEGIN_TIME, date.time)
                    //.putExtra(CalendarContract.EXTRA_EVENT_END_TIME, endMillis)
                    .putExtra(CalendarContract.Events.TITLE, eventLeague.homeTeamName+" vs "+eventLeague.awayTeamName)

            startActivity(it.context, intnt, intnt.extras)
        }
